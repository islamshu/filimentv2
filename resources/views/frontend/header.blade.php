@php
    ...
    $categories = \App\Models\Category::whereHas('subcategories.products', function ($query) use ($countryId) {
        if ($countryId) {
            $query->whereHas('countries', function ($q) use ($countryId) {
                $q->where('countries.id', $countryId);
            });
        } else {
            $query->whereRaw('0 = 1'); // لو ما في دولة → لا تعرض شي
        }
    })
        ->with([
            'subcategories' => function ($q) use ($countryId) {
                $q->whereHas('products', function ($query) use ($countryId) {
                    if ($countryId) {
                        $query->whereHas('countries', function ($qq) use ($countryId) {
                            $qq->where('countries.id', $countryId);
                        });
                    } else {
                        $query->whereRaw('0 = 1');
                    }
...
<header class="header" style="box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;">
    <div class="upper-bar"
        style="background-color: {{ get_general_value('primary_color') }};color:white;padding-top:.8rem;padding-bottom:.8rem;">
        <div class="container">
            <div class="row align-items-center">
                ...
                <div class="col-3 col-lg-4">
                    <div class="d-flex align-items-center">
                        @php
                            $country = \App\Models\Country::find(session('country_id'));
                        @endphp

                        @if ($country)
                            <div class="d-flex align-items-center">
                                <img loading="lazy" src="{{ asset('storage/' . $country->flag) }}" width="30"
                                    class="me-2" alt="">

                                <span class="me-2">|</span>

                                <span style="font-size: 13px;" class="me-4">
                                    {{ $country->currency }}
                                </span>
                            </div>
                        @endif
                        <a href="{{ route('page', 'term') }}" style="font-size: 13px;"
                            class="text-decoration-none text-white d-none d-lg-inline-block me-4">سياسة الخصوصية
                            والشروط والاحكام</a>
                        <a href="{{ route('page', 'pay') }}" style="font-size: 13px;"
                            class="text-decoration-none text-white d-none d-lg-inline-block ">طرق الدفع والأقساط</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="navbar navbar-expand-lg main_menu py-0">
        <div class="container-fluid my-0">
            <a class="navbar-brand d-md-none bg-danger d-none d-md-block" href="/">
                <img loading="lazy" src="{{ asset('storage/' . get_general_value('web_logo')) }}" height="60"
                    width="50" alt="" />
            </a>
                ...
                <div class="d-flex align-items-center ">
                    ...
                    <div class="">
                        <a class="navbar-brand d-md-none  " href="/">
                            ...
                            <img loading="lazy" src="{{ asset('storage/' . get_general_value('web_logo')) }}"
                    ...
                    <div class=" d-flex align-items-center ms-auto">
                        ...
                        <a href="#" class="nav-link position-relative p-0 d-md-none me-2"
                            style="width: 30px;height:30px;border-radius:50%;background-color:{{ get_general_value('primary_color') }};text-align:center;line-height:15px;display:flex;justify-content:center;align-items:center">
                        ...
                        <a href="{{ route('cart.index') }}" class="nav-link position-relative d-md-none p-0"
                            style="width: 30px;height:30px;border-radius:50%;background-color:{{ get_general_value('primary_color') }};text-align:center;line-height:15px;display:flex;justify-content:center;align-items:center">
                            ...
                            <span
                                style="    background-color: {{ get_general_value('primary_color') }} 
                                    ...
                                    font-size: 12px;
                                    padding: 0;"
                                class="position-absolute top-0 start-0 ms-1 mt-1 translate-middle badge rounded-pill text-white">
                                0
                            </span> </a>
                    </div>
                </div>
            ...
            <div class="collapse navbar-collapse my-1" id="navbarSupportedContent" style="padding-top: 15px;">
                <a class="navbar-brand d-md-block d-none ms-auto me-auto" href="/">
                    <img loading="lazy" src="{{ asset('storage/' . get_general_value('web_logo')) }}" height="50"
                        width="150" alt="" />
                </a>
                ...
                <form class="d-flex ms-auto me-auto searchForm" role="search">
                    <div class="input-group" style="position: relative;" dir="rtl">
                        ...
                        <input type="text" aria-label="Last name" class="form-control w-50 "
                            style="text-indent: 25px;border-radius:6px ! important;background-color:{{ get_general_value('primary_color') }};padding:10px;border:none ;"
                            placeholder="أدخل كلمة البحث" />
                        <i class="fa-solid fa-magnifying-glass"
                            style="position: absolute; right:15px; top:36%;color:#8b8b8b"></i>
                ...
                <ul class="navbar-nav me-auto mb-2 mb-lg-0 ms-auto me-auto">
                    <li class="nav-item mx-2 me-4 d-flex">
                        <a href="#" class="nav-link position-relative p-0"
                            style="width: 40px;height:40px;border-radius:50%;background-color:{{ get_general_value('primary_color') }};text-align:center;line-height:20px;display:flex;justify-content:center;align-items:center">
                    ...
                    <li class="nav-item mx-2 d-flex">
                        <a href="{{ route('cart.index') }}" class="nav-link position-relative p-0"
                            ...
                            <i class="fa-solid fa-bag-shopping" style="font-size: 20px;color:white"></i>
                            <span
                                style="background-color:  {{ get_general_value('primary_color') }}; border: 2px solid black; width: 18px; height: 18px; border-radius: 50% !important; font-size: 12px; padding: 0;"
                                class="position-absolute top-0 start-0 ms-1 mt-1 translate-middle badge rounded-pill text-white">
                                {{ count(session('cart', [])) }}
                            </span>
                        </a>
    ...
    <nav class="navbar navbar-expand-lg navbar-light navbar-dark shadow d-none"
        style="background-color: {{ get_general_value('primary_color') }};">
        <div class="container">
                ...
                <ul class="navbar-nav mr-auto mb-2 mb-lg-0">
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle text-white" href="#" data-bs-toggle="dropdown"
                            data-bs-auto-close="outside">قسم الالعاب</a>
    ...
    <aside>
        <div class="offcanvas offcanvas-bottom h-75 rounded-3 mx-1" tabindex="-1" id="offcanvasExample"
                        ...
                        <div class="accordion-item border-bottom">
                            <h2 class="accordion-header" id="flush-heading{{ $index }}">
                                <button class="accordion-button collapsed px-0" type="button"
                                    ...
                                    aria-expanded="false" aria-controls="section{{ $index }}">
                                    {{ $category->name }}
                                </button>
                            </h2>
                            <div id="section{{ $index }}" class="accordion-collapse collapse"
                                aria-labelledby="flush-heading{{ $index }}" data-bs-parent="#sections">
                                <div class="accordion-body">
                                    <div class="accordion accordion-flush" id="categories">
                                        @foreach ($category->subcategories as $subcategory)
                                            <div class="py-3 border-bottom">
                                                <a href="{{ route('category.slug', $subcategory->slug) }}"
    ...
    <aside>
        <div class="offcanvas offcanvas2 offcanvas-right h-75 rounded-3 mx-1 d-none d-lg-block"
            style="width: 30% !important;height: 100% !important;right: -5px;border-radius: 0 !important;"
            tabindex="-1" id="offcanvasExample2" aria-labelledby="offcanvasExampleLabel">
            <div class="offcanvas-header">
                <div class="offcanvas-title fw-bold">
                    فئات المنتجات
                </div>
    ...
    background-color: #ed4555 !important;opacity:1 !important"
                    class="btn-close bg-danger" data-bs-dismiss="offcanvas" aria-label="Close"></button>
            </div>
            <div class="offcanvas-body mt-2">
